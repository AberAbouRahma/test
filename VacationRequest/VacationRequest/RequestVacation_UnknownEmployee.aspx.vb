...
Public Class RequestVacation_UnknownEmployee
    ...
    Public Function GetEmployee(<QueryString("EmployeeCode")> employeeCode As String, <RouteData> employeeName As String) As IQueryable(Of Employee)
        ...
        If employeeCode <> "" Then
            ...
            If emp.vacType1Balanace = 0 And emp.vacType2Balanace = 0 Then
                MsgBox("Sorry cann not request vacation for " & emp.EmployeeName & ". No vacation balance left")
        ...
        Else
            query = db.Database.SqlQuery(Of Employee)("GetAllEmployees")
            If db.Employees.Count() <> 0 Then
                query = db.Database.SqlQuery(Of Employee)("GetEmpsWithVacationBalance")
                If query.Count = 0 Then ' No vacation balance for all employees
                    MsgBox("Sorry no vacation balance left for all employees in our database")
                    Response.Redirect("EmployeeList.aspx")
                End If
                currentEmpCode = query.FirstOrDefault().EmployeeCode
            End If
        End If
        Return query.AsQueryable()
    End Function

    Protected Sub Nametext_SelectedIndexChanged(sender As Object, e As EventArgs)
        Dim Nametext As DropDownList = vacationDetail.FindControl("Nametext")
        Dim value As String
        If (Not Nametext Is Nothing) Then
            value = Nametext.SelectedValue
            query = query.Where(CType(Function(p) p.EmployeeName = value, Func(Of Employee, Boolean)))
            Dim q As List(Of Employee) = query.ToList
            Dim emp As Employee = q.ElementAt(0)
            If emp.vacType1Balanace = 0 And emp.vacType2Balanace = 0 Then
                MsgBox("Sorry cann not request vacation for " & emp.EmployeeName & ". No vacation balance left")
            ...
            Dim requestText As TextBox = vacationDetail.FindControl("requestText")
            requestText.Text = emp.EmployeeCode & "_" & (emp.ApprovedVacationRequests + 1)
            ' Do the fields' clean up according with the selected employee data
            Dim vacTypeText As DropDownList = vacationDetail.FindControl("vacTypeText")
            vacTypeText.SelectedIndex = 0
